...
class SpecCreator:
    ...
    def create_spectrogram(self, audio_path):
        ...
        cmap.set_under('w')
        f, t, sxx = scipy.signal.spectrogram(w, fs=fs, window='hann', nperseg=int(fs / 12.5),
                                             noverlap=int(self.overlap * (fs / 12.5)), mode='psd', nfft=16000)
        sxx_db = 10 * np.log10(abs(sxx[:1500, :]) / 2 * 10e-5)

        plt.axis('off')
        plt.imshow(sxx_db[::-1, :],  cmap=cmap, extent=(0, dur * self.sec_size, 0, self.sec_size * 2),
                   norm=mpl.colors.Normalize(vmin=-50, vmax=0, clip=False))
        plt.savefig(osp.join(self.out_path, '%s.jpeg' % audio_name))
        plt.close()
